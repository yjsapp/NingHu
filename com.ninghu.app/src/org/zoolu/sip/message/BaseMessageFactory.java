...
//   ToHeader toh=req.getToHeader();
   ...
   /*******修改*********/
 //  if (content_type==null) resp.setBody(body);修改
 //  else resp.setBody(content_type,body);修改
   if(SipInfo.query_response){    //若为邀请视频query，则回复query_response的XML消息体
 	  resp.setBody(SipInfo.QueryMediaResponseXML);//添加
 	 
   }
   else if(SipInfo.request_media){  //若为invite之后的回复，加上media消息体
 	  resp.setBody(SipInfo.RequestMediaResponseXML);
 	  SipInfo.request_media = false;
   }
   ...
   public static Message createResponse1(Message req, int code, String reason, NameAddress contact,String status)
   {  //String reason=SipResponses.reasonOf(code);
      String localtag=null;
      if (req.createsDialog() && !req.getToHeader().hasTag())
      {  if (SipStack.early_dialog || (code>=200 && code<300)) localtag=SipProvider.pickTag(req);
      }
      return createResponse1(req,code,reason,localtag,contact,null,null,status);
   }
   
   
   public static Message createResponse1(Message req, int code, String reason, String local_tag, NameAddress contact, String content_type, String body,String status)
   {   Message resp=new Message(); 
	  resp.setRemoteAddress(SipInfo.serverip);//修改 添加
	  resp.setRemotePort(6060);//修改 添加
   resp.setStatusLine(new StatusLine(code,reason));
//   resp.setVias(req.getVias());
   resp.addHeader(AudioInfo.viash, false);     //直接调用BaseMessage中的这个函数设置via头
   if (code>=180 && code<300 && req.hasRecordRouteHeader())
      resp.setRecordRoutes(req.getRecordRoutes());
   
//   ToHeader toh=req.getToHeader();
   ToHeader toh=AudioInfo.toh;
   if (local_tag!=null)
      toh.setParameter("tag",local_tag);
   resp.setCallIdHeader(req.getCallIdHeader());
   resp.setFromHeader(req.getFromHeader());
   resp.setFromHeader(AudioInfo.fromh);
   resp.setToHeader(toh);
 ...
 //  if (content_type==null) resp.setBody(body);修改
 //  else resp.setBody(content_type,body);修改
   if(status.equals("OK")){    //若为邀请视频query，则回复query_response的XML消息体
 	  resp.setBody(AudioInfo.AudioResponse_OK);//添加
 	 
   }
   else if(status.equals("Error")){  //若为invite之后的回复，加上media消息体
 	  resp.setBody(AudioInfo.AudioResponse_Error);
   ...
   public static Message createResponse2(Message req, int code, String reason, NameAddress contact)
   {  //String reason=SipResponses.reasonOf(code);
      String localtag=null;
      if (req.createsDialog() && !req.getToHeader().hasTag())
      {  if (SipStack.early_dialog || (code>=200 && code<300)) localtag=SipProvider.pickTag(req);
      }
      return createResponse2(req,code,reason,localtag,contact,null,null);
   }
   
   public static Message createResponse2(Message req, int code, String reason, String local_tag, NameAddress contact, String content_type, String body)
   {   Message resp=new Message(); 
	  resp.setRemoteAddress(SipInfo.serverip);//修改 添加
	  resp.setRemotePort(6060);//修改 添加
   resp.setStatusLine(new StatusLine(code,reason));
//   resp.setVias(req.getVias());
   resp.addHeader(Gongdan.viash, false);     //直接调用BaseMessage中的这个函数设置via头
   if (code>=180 && code<300 && req.hasRecordRouteHeader())
      resp.setRecordRoutes(req.getRecordRoutes());
   
//   ToHeader toh=req.getToHeader();
   ToHeader toh=Gongdan.toh;
   if (local_tag!=null)
      toh.setParameter("tag",local_tag);
   resp.setCallIdHeader(req.getCallIdHeader());
   resp.setFromHeader(req.getFromHeader());
   resp.setFromHeader(Gongdan.fromh);
   resp.setToHeader(toh);
 ...
 //  else resp.setBody(content_type,body);修改
 	  ...
 	  resp.setBody(Gongdan.GongDanResponse_OK);//添加
