...
public class RTPSession {
	 ...
	 protected int payloadType = 0;    //修改
	 ...
	 public RTPSession(DatagramSocket rtpSocket, DatagramSocket rtcpSocket) {
		 mcSession = false;
		 rtpSock = rtpSocket;
		 try{this.generateCNAME();}catch(Exception e){e.printStackTrace();}
		 try{this.generateSsrc();}catch(Exception e){}
	//	 this.rtcpSession = new RTCPSession(this,rtcpSocket);//修改
	 ...
	 public long[] sendData(byte[] buf) {
		 byte[][] tmp = {buf}; 
		 long[][] ret = this.sendData(tmp, null, null, 0, null);
	...
	//	this.rtcpSession.senderThrd.interrupt();//修改
	...
	private void generateSeqNum() {
		if(this.random == null)
			try{createRandom();}catch(Exception e){}
	...
	protected void resolveSsrcConflict() {
		...
		if(this.conflictCount < 5) {
			...
			try{generateSsrc();}catch(Exception e){}
